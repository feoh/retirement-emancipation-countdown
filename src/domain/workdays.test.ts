...
describe("countWorkingDays", () => {
  it("counts a single Mon-Fri week", () => {
    expect(
      countWorkingDays(MONDAY, new Date(2026, 5, 8), DEFAULT_WORKING_WEEKDAYS),
    ).toBe(5);
  });
  ...
  it("excludes the retirement day itself", () => {
    ...
    expect(
      countWorkingDays(MONDAY, new Date(2026, 5, 5), DEFAULT_WORKING_WEEKDAYS),
    ).toBe(4);
  });

  it("includes the start day when it is a working day", () => {
    expect(
      countWorkingDays(MONDAY, new Date(2026, 5, 2), DEFAULT_WORKING_WEEKDAYS),
    ).toBe(1);
  });
  ...
  it("is zero when the target is today or earlier", () => {
    ...
    expect(
      countWorkingDays(MONDAY, new Date(2026, 4, 1), DEFAULT_WORKING_WEEKDAYS),
    ).toBe(0);
  });
  ...
  it("handles a single selected weekday over four weeks", () => {
    expect(countWorkingDays(MONDAY, new Date(2026, 5, 29), MONDAY_ONLY)).toBe(
      4,
    );
  });
  ...
  it("handles a partial trailing week", () => {
    ...
    expect(
      countWorkingDays(MONDAY, new Date(2026, 5, 10), DEFAULT_WORKING_WEEKDAYS),
    ).toBe(7);
  });
});
...
describe("estimateWorkingTime", () => {
  ...
  it("measures vacation proration from tomorrow", () => {
    const result = estimateWorkingTime(
      MONDAY,
      new Date(2026, 5, 3),
      DEFAULT_WORKING_WEEKDAYS,
    ...
    expect(result.rawWorkingDays).toBe(1);
    expect(result.vacationDays).toBe(0);
  });
